...
InitGraphics()
{
	...
	//Keytime Animation start

	//Add AddTimeValue(Time, Value); for the quantities.
	//Keep t=0 and t=10 value same.
	//Make sure you have 6 Keytime frames for each type of animation
	//Add more needed Animation other than the ones mentioned below

	//Example with Translation of object along x axis
	...
	//	B.Init();
	//	B.AddTimeValue(0.0, 0.0);
	//	B.AddTimeValue(10.0, 0.0);
	...
	for (float t = 0; t <= 10; t += 0.1) {
		float angle = t / 10.0f * 2 * M_PI; // Convert time to angle
		float xPos = 25.0f * cos(angle); // Adjust radius and use cos for x-axis
	...
	for (float t = 0; t <= 10; t += 0.1) {
		float angle = t / 10.0f * 2 * M_PI; // Convert time to angle
		float yPos = 25.0f * sin(angle); // Adjust radius and use sin for y-axis
	...
	Xeye.Init();
	Xeye.AddTimeValue(0, 0);
	Xeye.AddTimeValue(10, 0);

	Yeye.Init();
	Yeye.AddTimeValue(0, 0);
	Yeye.AddTimeValue(10, 0);

	Zeye.Init();
	Zeye.AddTimeValue(0, 15);
	Zeye.AddTimeValue(10, 15);
