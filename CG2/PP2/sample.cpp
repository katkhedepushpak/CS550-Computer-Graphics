...
const char* WINDOWTITLE = "OpenGL / GLUT Sample -- Pushpak Katkhede";
...
GLuint HorseList;				//Horse 
GLuint CircleList;				// Circle List
...
Display()
{
	...
	if (cc == 0)
		...
		gluLookAt(5.f, 2.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f);
	...
	float Theta = 360.f * Time;
	float Theta2 = 45 * sin(2 * (2 * 3.14 * Time));
	float Theta3 = 0.4 * sin(2 * (2 * 3.14 * Time));
	// draw the box object by calling up its display list:
	//Horse1
	glPushMatrix();
	glTranslatef(0, Theta3, 0);
...
InitLists()
{
	...
	// Creating the orbit for the horse to run along
	...
	/*	for (int i = 0; i < 360; i++)
		{
			glColor3f(1, 1, 0);
			float theta = 3.0f * 3.1415926f * float(i) / float(360);
			float x = 2 * cos(theta);
			float y = 2 * sin(theta);
			glVertex3f(x, 0, y);
		}
